...
const Product = (props) => {
    ...
    return (
        <>
            <ProductCard className="bg-dark text-light border border-light" key={props.ind}>

                <ProductImg src={props.data.donut_img} />
                <ProductInfo>
                    <div className="row w-100">
                        <ProductName className="col-md-5">{props.data.donut_name}</ProductName>
                        {
                            currentUser.favorites.filter(item => item.donut_id === props.data.donut_id).length > 0 ?

                                <svg onClick={removeFromFavoritesHandler} role="button" xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="red" class="bi bi-heart-fill" viewBox="0 0 16 16">
                                    <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" />
                                </svg>
                                :
                                <svg onClick={addToFavoritesHandler} role="button" xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-heart" viewBox="0 0 16 16">
